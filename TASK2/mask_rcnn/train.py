...
def create_model(num_classes, load_pretrain_weights=True):
    ...
    backbone = resnet50_fpn_backbone(pretrain_path="backbone/resnet50.pth", trainable_layers=3)

    model = MaskRCNN(backbone, num_classes=num_classes)

    if load_pretrain_weights:
        # coco weights url: "https://download.pytorch.org/models/maskrcnn_resnet50_fpn_coco-bf2d0c1e.pth"
        weights_dict = torch.load("backbone/maskrcnn_resnet50_fpn_coco.pth", map_location="cpu")
...
def main(args):
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        loss_history, det_info, seg_info = utils.evaluate(model, val_data_loader, device=device)
        tb_writer.add_scalar('test loss', loss_history['loss_classifier'], epoch)
        ...
        tb_writer.add_scalar('mAP', det_info[1], epoch)
        # save weights
        save_files = {
...
if __name__ == "__main__":
    ...
    # 训练数据集的根目录
    parser.add_argument('--data-path', default='/home/Leo/Neural_Network/PJ2/PJ2_DATA620004-main/VOCdevkit', help='dataset')
    # 检测目标类别数(不包含背景)
    parser.add_argument('--num-classes', default=20, type=int, help='num_classes')
    # 文件保存地址
    parser.add_argument('--output-dir', default='./save_weights_1', help='path where to save')
    ...
    parser.add_argument("--tensorboard_log", type=str, default='./logs_voc_frompretrain')
